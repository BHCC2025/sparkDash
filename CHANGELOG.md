...
### Added
- **Multi-GPU hosts** — a dedicated GPU host with several NVIDIA cards now reports every card. `metrics.gpu` keeps its shape and stays the aggregate the Overview and alerts already read (hottest card, busiest card, power and VRAM summed); it gains `gpu.gpus[]` with one entry per physical GPU: `index`, `name`, `uuid`, usage, temperature, power, VRAM, throttle state and the processes holding memory on that card. On a unit with two or more GPUs the GPU panel adds a block per card (throttle chip, usage and temperature sparklines, power, VRAM bar) above the all-cards VRAM total, and the unit header names every card (`2× NVIDIA GeForce RTX 5080`, or `NVIDIA GeForce RTX 5080 + RTX 5060 Ti`). Single-GPU units, including every DGX Spark, render exactly as before.

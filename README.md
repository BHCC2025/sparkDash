...
| **Multi-GPU hosts** | A dedicated GPU host with several NVIDIA cards reports each one: the GPU panel adds a per-card block (usage · temp · power, VRAM bar, throttle chip) and the API exposes `gpu.gpus[]`. The headline `gpu` numbers stay a fleet-wide aggregate so Overview cards and alerts need no change |
